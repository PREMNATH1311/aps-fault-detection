...
class DataValidation:
    def __init__(self, 
        ...
        try:
            logging.info(f"{'<<'*25} Data Validation {'>>'*25}")
    ...
    def drop_missing_values_columns(self, 
                                    df: pd.DataFrame, 
                                    report_key_name: str) -> Optional[pd.DataFrame]:
        """
        This function will drop columns which contain missing values more than the specified threshold.
        df: Accepts a pandas DataFrame
        threshold: Percentage criteria to drop a column
        =====================================================================================
        returns Pandas DataFrame if at least a single column is available after dropping missing columns else None
        """
        try:
            threshold = self.data_validation_config.missing_threshold
            null_report = df.isna().sum() / df.shape[0]
            
            # Selecting column names which contain null values above the threshold
            ...
            # Return None if no columns are left
            if len(df.columns) == 0:
                return None
            return df
    ...
    def data_drift(self, 
        ...
        try:
            ...
            for base_column in base_columns:
                ...
                # Null hypothesis is that both column data are drawn from the same distribution
    ...
    def initiate_data_validation(self) -> artifact_entity.DataValidationArtifact:
        try:
            ...
            train_df = self.drop_missing_values_columns(df=train_df, report_key_name="missing_values_with_train_dataset")            
            logging.info(f"Drop null values columns from test df")
            test_df = self.drop_missing_values_columns(df=test_df, report_key_name="missing_values_with_test_dataset")
            ...
            logging.info(f"Check if all required columns are present in train df")
            ...
            logging.info(f"Check if all required columns are present in test df")
