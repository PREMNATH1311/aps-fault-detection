...
class ModelEvaluation:
    ...
    def initiate_model_evaluation(self) -> artifact_entity.ModelEvaluationArtifact:
        try:
            ...
            logging.info("Checking if a saved model exists")
            latest_dir_path = self.model_resolver.get_latest_dir_path()
            if latest_dir_path is None:
                model_eval_artifact = artifact_entity.ModelEvaluationArtifact(is_model_accepted=True, improved_accuracy=None)
                logging.info(f"Model evaluation artifact: {model_eval_artifact}")
                return model_eval_artifact  # Ensure to return here

            # Finding paths for transformer, model, and target encoder
            logging.info("Finding paths for transformer, model, and target encoder")
            transformer_path = self.model_resolver.get_latest_save_transformer_path()
            model_path = self.model_resolver.get_latest_model_path()
            target_encoder_path = self.model_resolver.get_latest_target_encoder_path()
            
            logging.info("Loading previous trained objects of transformer, model, and target encoder")
            # Load previous trained objects
            transformer = load_object(file_path=transformer_path)
            model = load_object(file_path=model_path)
            target_encoder = load_object(file_path=target_encoder_path)
            
            logging.info("Loading currently trained model objects")
            # Load current trained model objects
            ...
            logging.info("Calculating accuracy using current trained model")
...
#             #which model is best trianed or the model from saved model folder
#             logging.info("if saved model folder has model the we will compare which model is the best trained or the model from saved model folder")
#             latest_dir_path=self.model_resolver.get_latest_dir_path()
#             if latest_dir_path==None:
#                 model_eval_artifact=artifact_entity.ModelEvaluationArtifact(is_model_accepted=True,improved_accuracy=None)
                
#                 logging.info(f"Model evvaluation artifact:{model_eval_artifact}")
#                 return model_eval_artifact
            
#             #Finding location of transofomer model and target encoder
#             logging.info("Finding location of transformer model and target encoder")
#             transformer_path=self.model_resolver.get_latest_save_transformer_path()
#             model_path=self.model_resolver.get_latest_model_path()
#             target_encoder_path=self.model_resolver.get_latest_target_encoder_path()
            
#             logging.info("Previous trained objjects of transformer, model and target encoder")
...
#             previous_model_score=f1_score(y_true=y_true,y_pred=y_pred)
#             logging.info(f"Accuracy using previous trained mode:{previous_model_score}")
